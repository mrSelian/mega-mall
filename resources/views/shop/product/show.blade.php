<x-app-layout>
    <x-slot name="header">
        <x-jet-nav-link href="{{ route('seller_page',$product->getSellerId()) }}"
                        :active="request()->routeIs('seller_page')">
                ...
                <div class="row details_row">
                    ...
                    <div class="col-lg-6">
                        <div class="px-2 details_content">
                            <div class="details_name uppercase tracking-wide mb-3 text-lg font-semibold text-gray-700 "
                                 data-id="{{$product->getId()}}">{{$product->getName()}}</div>
                            <!-- In Stock -->
                            <div class="in_stock_container tracking-wide font-semibold  mb-1 ">
                                ...
                                @elseif($product->getAmount() > 0)
                                    <span class=""
                                          style="color: green">В наличии: <br>{{$product->getAmount()}} шт</span>
                                @else
                                    <span style="color: red">Отсутствует</span>
                                @endif
                            </div>
                            <div class="details_price text-3xl text-gray-900 mb-3">{{$product->getPrice()}}&#8381;
                            </div>
                            ...
                            <!-- ProductModel Quantity -->
                                <div class="product_quantity_container">
                                    <div class="product_quantity clearfix">
                                        <label for="amount"></label><input name="amount" id="amount" pattern="[0-9]*"
                                                                           type="text" value="1">
                                        <span>шт</span>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-offset-3 col-sm-6">
                                        @if($product->getAmount() < 1 or $product->isDeleted())
                                            <button type="submit"
                                                    class="text-xl w-full bg-gray-800 text-white px-4 py-2 border disabled:opacity-40 rounded-md  "
                                                    disabled>
                                                <i class="fa fa-plus"></i> Добавить в корзину
                                            </button>
                                        @else
                                            <button type="submit"
                                                    class="text-xl w-full bg-indigo-500 text-white px-4 py-2 border rounded-md hover:bg-white hover:border-indigo-500 hover:text-black">
                                                <i class="fa fa-plus"></i> Добавить в корзину
                                            </button>
                                        @endif
                                    </div>
